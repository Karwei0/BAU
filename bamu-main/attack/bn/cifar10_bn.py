...
if m_num != 0:
    if not os.path.exists(path + "mitigation_idx.npy"):
        # 从 posion 中选择部分作为 mitigation
        if m_num <= p_num:
            mitigation_idx = random.sample(list(backdoor_indexes), m_num)
        else:
            # 不够的话 选择 剩下非 label 标签的非 posion 作为 mitigation
...
for i in backdoor_indexes:
    ...
    train_dataset.data[i,-4:,-4:,:] = a * 255 # 选择最后四行 和 四列注入 trigger （右下角）
...
if m_num != 0:
    ...
    for i in range(len(x_mitigation)):
        x_mitigation[i,-4:,-4:,:] = 255 # a larger alpha => 1

    if not os.path.exists(path + "indexes_sample.npy"):
        indexes = list(range(len(train_dataset.targets) + len(y_mitigation))) # 建立索引表 掺入了 mitigation
...
if args.requests != 0:
    ...
    if len(files) == 1:
        all_requests = random.sample(list(mitigation_indexes), args.requests) # 随机选择 mitigation 中的部分作为 unlearn
    else:
        files.sort(key=lambda x: int(x.split("-")[1][:-4]))
        # 读取每个 shard 的 request 请求
...
# train
# posion and mitigation 已经掺入 train_set
sisa_train(args, train_data, train_label, train_kwargs)

# 先测试正常标签的 然后对非posion的数据植入后门 进行测试
# test
print("normal data")
